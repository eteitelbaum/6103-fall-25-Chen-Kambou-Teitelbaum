...
Top-right visualization on the dashboard page.
...
def create_viz_2(df_filtered, selected_indicator=None):
    """
    Create the second visualization (placeholder).
    This will be displayed in the top-right position on the dashboard.
    ...
    fig.add_annotation(
        text=f"Visualization 2 - To be implemented\nSelected indicator: {indicator_label}",
    ...
    fig.update_layout(
        title=f"Visualization 2: {indicator_label}",
        xaxis=dict(visible=False),
        yaxis=dict(visible=False),
        template="plotly_white"
    )
    
    return dcc.Graph(figure=fig, id="viz-2")
